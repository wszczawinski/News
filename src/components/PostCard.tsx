  ...
  return (
    <Card key={post.id} className="md:h-[162px]">
      <div className="flex flex-col md:flex-row ">
        <img
          className="h-[140px] md:h-[160px] md:w-[220px] object-cover rounded-t-lg md:rounded-l-lg md:rounded-r-none"
          src={randomImg}
        />
        <CardContent className="flex-1 pt-1 pb-2 px-2 flex flex-col gap-1 justify-between ">
          <CardHeader className="p-0">
            ...
            <CardDescription className="m-0 line-clamp-4">
              {post.content}
            </CardDescription>
          </CardHeader>
          ...
          <CardFooter className="flex justify-between p-0 text-xs">
            ...
            <Link to="/post/$postId" params={{ postId: post.id }}  >
              <Button
                size={"sm"}
                variant={"outline"}
                className="cursor-pointer text-sky-600 hover:opacity-75 hover:text-sky-600"
              >
                <ChevronRight />
              </Button>
            </Link>
